...
/**
 * Filtering, sorting and paging executed against a real PostgreSQL database
 * (Testcontainer), through {@link ItemRepository} directly. Every test loads
 * its own small, deterministic {@code @DataSet} fixture and cleans it up
 * afterwards, so tests never depend on execution order or on each other's data.
 */
class ItemRepositoryIT extends IntegrationTestBase {
    ...
    @Test
    @DataSet(value = "datasets/pagination-items.yml", cleanBefore = true, cleanAfter = true)
    ...
    @Test
    @DataSet(value = "datasets/search-items.yml", cleanBefore = true, cleanAfter = true)
    ...
    void textSearchToleratesNullDescription() {
        ...
        // (Postgres: NULL LIKE anything is NULL, not an error, but a predicate
    ...
    @Test
    @DataSet(value = "datasets/items-with-equal-created-at.yml", cleanBefore = true, cleanAfter = true)
